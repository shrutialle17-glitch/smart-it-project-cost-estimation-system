...
const getStats = async (req, res, next) => {
  try {
    ...
    const thisMonthEstimations = await Estimation.countDocuments({ createdAt: { $gte: startOfMonth } });
    const thisMonthClients = await User.countDocuments({ role: 'client', createdAt: { $gte: startOfMonth } });

    const byProjectType = await Estimation.aggregate([
      { $group: { _id: '$projectType', count: { $sum: 1 }, totalValue: { $sum: '$calculation.totalCost' } } },
      { $sort: { count: -1 } },
    ]);
    ...
    sendResponse(res, 200, 'Admin stats retrieved', {
      totalClients, totalEstimations, totalRevenue: Math.round(totalRevenue * 100) / 100,
...
const getClients = async (req, res, next) => {
  try {
    ...
    const clients = await User.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit);
    const clientsWithEstCount = await Promise.all(
      clients.map(async (client) => {
        const estCount = await Estimation.countDocuments({ client: client._id });
        return { ...client.toJSON(), estimationCount: estCount };
      })
    );
    sendResponse(res, 200, 'Clients retrieved', { clients: clientsWithEstCount }, { page, limit, total, pages: Math.ceil(total / limit) });
  } catch (error) { next(error); }
};
...
const getClient = async (req, res, next) => {
  try {
    ...
    const estimations = await Estimation.find({ client: client._id }).sort({ createdAt: -1 });
...
const getAllEstimations = async (req, res, next) => {
  try {
    ...
    const estimations = await Estimation.find(filter).populate('client', 'name email company').populate('projectType', 'name slug icon').sort({ createdAt: -1 }).skip(skip).limit(limit);
    sendResponse(res, 200, 'Estimations retrieved', { estimations }, { page, limit, total, pages: Math.ceil(total / limit) });
  } catch (error) { next(error); }
};
...
module.exports = { getStats, getClients, getClient, getAllEstimations, updateEstimationStatus };
